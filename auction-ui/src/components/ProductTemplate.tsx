    ...
    <section className="product-details-wrp">
      ...
      {data?.id ? (
        <div className="container">
          ...
          <div className="bid-container container">
            <div className="row">
              ...
              <div
                className="d-flex flex-column align-items-center col-md-4 rounded-3 border py-4 px-0"
                style={{ backgroundColor: '#fff' }}
              >
                {new Date(data.auction_start_date) > new Date() ? (
                  <p className="end-date">
                    Auction Starting Date: <b>{data.auction_start_date ? dateFormat(new Date(data.auction_start_date)) : <></>}</b>
                  </p>
                ) : (
                  <p className="end-date">
                    Bid End Date: <b>{data.expiry_at ? dateFormat(new Date(data.expiry_at)) : <></>}</b>
                  </p>
                )}
                ...
                <div className="d-flex countdown">
                  {data.expiry_at ? (
                    <Countdown
                      date={new Date(data.auction_start_date) > new Date() ? new Date(data.auction_start_date) : new Date(data.expiry_at)}
                      renderer={renderer}
                    >
                      <Completionist />
                    </Countdown>
                ...
                <div className="d-flex flex-column align-items-center justify-content-center w-75 current-bid-container my-4 py-3 px-5">
                  <p className="fs-6 current-bid-title m-0">Current bid</p>
                ...
                {winnerUser ? (
                  <div className="text-center">
                    ...
                    {route_id ? <Link href={'/winner-payment/' + route_id}>Click Here to checkout</Link> : ''}
                    <hr />
                  </div>
                ) : (
                  ''
                )}
                {data.is_expired || new Date(data.auction_start_date) > new Date() ? (
                  <>
                    <p>
                      <Link href="/collections">Checkout more auctions </Link>
                    </p>
                  </>
                ) : (
                  <>
                    ...
                    <div className="container px-5">
                      <input
                        ...
                        defaultValue={currentbid.bid ? currentbid.bid : data.starting_bid}
                        className="form-control mb-3"
                        id="exampleInputEmail1"
                        aria-describedby="emailHelp"
                      />
                    </div>
                    <div className="cb-bid-note">Bids require 20,000$ deposit, refunded after auction</div>
                    <div className="w-100 border-top d-flex align-items-center justify-content-center mt-4">
                      <button onClick={handleSubmit} className="btn btn-primary w-75 cb-bid-btn mt-4 py-2">
                        Place bid
                      </button>
                    </div>
