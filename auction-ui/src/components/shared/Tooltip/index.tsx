...
export const TooltipTrigger = React.forwardRef<HTMLElement, React.HTMLProps<HTMLElement> & { asChild?: boolean }>(function TooltipTrigger(
  ...
  // `asChild` - pass any element as the anchor
  ...
  return (
    <button
      ref={ref}
      // style trigger based on the state
      data-state={context.open ? 'open' : 'closed'}
...
export const TooltipContent = React.forwardRef<HTMLDivElement, React.HTMLProps<HTMLDivElement>>(function TooltipContent(props, propRef) {
  const context = useTooltipContext()
