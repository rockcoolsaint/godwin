...
import { putFormData } from "../utils/fetch";
...
const UserProfileContent = ({ data, userhistory, setData }: Props) => {
  ...
  const handleChangeFile = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (e.target.files) {
      ...
      getIdTokenClaims().then(async (data1: any) => {
        ...
        if (data1.__raw) {
          try {
            const result = await putFormData(url("/api/profile/"), formData, {
  ...
  return (
    <div className="user-profiletab-contents">
      ...
      <div
        className={
          !data ? "user-profile-content invisible" : "user-profile-content"
        }
      >
          ...
          <div className="row">
            <div className="col-md-5 col-sm-5">
              <div className="profile-name">
                <h4>
                  {data?.date_joined
                    ? dateFormat(new Date(data?.date_joined))
                    : dateFormat(new Date())}
                </h4>
                <h3>User Account</h3>
              </div>
            ...
            <div className="col-md-7 col-sm-7">
              <div className="profile-actbar">
                <ul>
                  ...
                  />
                  <li>
                    <label htmlFor="profile_pic" className="profileimg">
                      <i className="fa fa-pencil"></i>
                      <img
                        src={
                          data?.uploaded_profile
                            ? data?.uploaded_profile
                            : data?.profile_pik
                            ? data?.profile_pik
                            : "https://auctions.rigly.io/media/auction_profile_images/profile_dummy.jpg"
                        }
                        alt=""
                      />
                    </label>
            ...
            <div className="col-md-6 col-sm-6">
              <div className="contact-probx">
                ...
                <div className="pro-cnt">
                  <h5>Mobile</h5>
                  <h6>
                    {data?.phone_number ? data.phone_number : "Not found"}
                  </h6>
                </div>
              </div>
            </div>
      ...
      <div className="mt-3 text-center">
        <div className="any-que-wrp">
          ...
          <p>
            Both referral and referee with get 10% discount on fee charged.{" "}
          </p>

          <div className="invoice-wrap justify-content-center">
            <div className="box invoice" ref={invoice} id="invoice">
              {data?.refer_code}
            </div>
            <span
              id="invoice-copy"
