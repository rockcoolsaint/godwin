  ...
  return (
    <>
      <Alert show={show} variant="success" className="mb-3">
        ...
        <p>Thanks, your account is now activated to place bids.</p>
        <br />
...
const SignUpPayment = () => {
  ...
  useEffect(() => {
    ...
    getIdTokenClaims().then(async (data1: any) => {
      if (data1.__raw) {
        try {
          const data = await post(url("/api/create-user/"), data1);
          console.log(data);
          setTimeout(() => {
            setshowContent(true);
          }, 1000);
        ...
        .then((data) => {
          if (data.data) {
            ...
            setTimeout(function checkPayment() {
              ...
              fetch(
                `https://api.opennode.com/v2/charge/${charge_id}`,
                checkOptions
              )
                .then((response) => response.json())
                .then((data) => {
    ...
    <div className={showContent ? "container customer register" : "d-none"}>
      ...
      <div id="content" className="content">
        ...
        <div className="content-banner">
          <p>
            Please scan QR code to pay <b>20 sats</b> to create your account
          </p>
          <i>
            This paywall helps us make sure your bids are real. You will create
            your account after payment.
          </i>
        </div>
        ...
        <section className="paywall-content">
          ...
          <div className="paywall-note">
            ...
            <div className="paywall-note-text">
              This paywall requires Lightning. If you need a Lightning wallet,
              check out
              <Link
                to="https://www.walletofsatoshi.com/"
                target="_blank"
              ...
              or
              <Link
                to="https://bluewallet.io/"
                target="_blank"
                rel="noopener noreferrer"
              >
                Blue wallet
              </Link>
              . Feel free to hit the Chat button if you need help!
            </div>
          </div>
        </section>
      </div>
