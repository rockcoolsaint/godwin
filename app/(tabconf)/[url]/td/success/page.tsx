  ...
  return (
    <Container className="flex h-screen flex-col items-center justify-start bg-slate-50 pt-4 sm:pt-12">
      <div className="mt-10 flex flex-col items-center justify-center">
        <BoltIcon
          className={clsx(
            loading ? 'animate-ping text-gray-500' : proxyStatus.order.status === 'delivery_ended' ? 'text-green-400' : 'text-yellow-400',
            '0 mb-4 h-24 w-24',
          )}
        />
        <h1 className="mb-2 text-4xl">
          {loading ? 'Hashrate initializing' : proxyStatus.order.status === 'delivery_ended' ? 'Hashrate delivered' : 'Hashrate up'}
        </h1>
      </div>
      ...
      {loading ? (
        ...
        <>
          ...
          <dl
            className={clsx('mx-auto grid grid-cols-1 gap-px rounded-md border border-black/[0.05] sm:grid-cols-2 lg:grid-cols-4', {
              'mt-8': proxyStatus.order.status !== 'delivery_ended',
            })}
          >
            <div className="flex flex-wrap items-baseline justify-between gap-x-4 gap-y-1 bg-white px-4 py-4 sm:px-6 sm:py-8 xl:px-8">
            ...
            <div className="flex flex-wrap items-baseline justify-between gap-x-4 gap-y-1 bg-white px-4 py-4 sm:px-6 sm:py-8 xl:px-8">
              ...
              <dd className="w-full flex-none break-all text-sm font-medium tracking-tight text-gray-900">
                <dd className="w-full flex-none break-all text-sm font-medium tracking-tight text-gray-900">
                  {proxyStatus.payment?.status === 'paid' && <span className="text-sm font-medium uppercase text-green-700 ">Paid</span>}
            ...
            <div className="flex flex-wrap items-baseline justify-between gap-x-4 gap-y-1 bg-white px-4 py-4 sm:px-6 sm:py-8 xl:px-8">
              ...
              <dd className="w-full flex-none break-all text-sm font-medium tracking-tight text-gray-900">{proxyStatus.pool_user?.pool}</dd>
            </div>
            <div className="flex flex-wrap items-baseline justify-between gap-x-4 gap-y-1 bg-white px-4 py-4 sm:px-6 sm:py-8 xl:px-8">
